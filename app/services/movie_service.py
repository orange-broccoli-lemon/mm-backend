...
class MovieService:
    ...
    async def get_movie_by_tmdb_id(self, tmdb_id: int) -> Optional[Movie]:
        try:
            print(f"DB 조회 시작 - TMDB ID: {tmdb_id}")
            
            stmt = select(MovieModel).where(MovieModel.tmdb_id == tmdb_id)
    ...
    async def save_movie_from_tmdb_data(self, tmdb_data: dict) -> Movie:
        try:
            ...
            print(f"TMDB 데이터 저장 시작 - ID: {tmdb_id}")
            ...
            print(f"DB 저장 완료 - 내부 ID: {movie_model.movie_id}")
            return Movie.from_orm(movie_model)
