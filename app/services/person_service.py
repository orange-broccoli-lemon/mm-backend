...
class PersonService:
    ...
    async def _update_person_details_from_tmdb(
        self, person_id: int, existing_person: Optional[PersonModel]
    ...
    async def _ensure_movie_basic_info(
        self, movie_id: int, title: str, poster_path: Optional[str], release_date: Optional[str]
    ):
    ...
    async def get_person_credits(self, person_id: int) -> PersonCreditsResponse:
        ...
        try:
            ...
            acting_stmt = (
                ...
                .where(
                    and_(
                        MovieCastModel.person_id == person_id, MovieCastModel.department == "Acting"
                    )
                )
