...
class UserFollowService:
    ...
    async def get_followers(self, user_id: int, current_user_id: Optional[int] = None, skip: int = 0, limit: int = 20) -> FollowListResponse:
        """팔로워 목록 조회"""
        try:
            # 팔로워 목록 쿼리
            stmt = select(
    ...
    async def get_following(self, user_id: int, current_user_id: Optional[int] = None, skip: int = 0, limit: int = 20) -> FollowListResponse:
        """팔로잉 목록 조회"""
        try:
            # 팔로잉 목록 쿼리
            stmt = select(
