...
from app.schemas.genre import Genre, GenreWithMovieCount, GenreListResponse, GenreMovieListResponse, GenreStatsResponse
...
class GenreService:
    ...
    async def get_movies_by_genre(self, genre_id: int, skip: int = 0, limit: int = 20) -> GenreMovieListResponse:
        """특정 장르의 영화 목록 조회"""
        try:
            # 장르 정보 조회
            ...
            stmt = select(MovieModel).join(
                MovieGenreModel, MovieModel.movie_id == MovieGenreModel.movie_id
            ).where(
                MovieGenreModel.genre_id == genre_id
            ).order_by(desc(MovieModel.average_rating)).offset(skip).limit(limit)
            ...
            # 총 영화 수
            total_stmt = select(func.count(MovieModel.movie_id)).join(
                MovieGenreModel, MovieModel.movie_id == MovieGenreModel.movie_id
    ...
    async def get_popular_genres(self, limit: int = 10) -> List[GenreWithMovieCount]:
        ...
        try:
            stmt = select(
                ...
                desc(func.count(MovieGenreModel.movie_id))
            ).limit(limit)
            
            result = self.db.execute(stmt)
            popular_data = result.all()
            
            return [GenreWithMovieCount(
                genre_id=row.genre_id,
                name=row.name,
