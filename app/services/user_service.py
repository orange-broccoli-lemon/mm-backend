...
class UserService:
    ...
    async def _get_counts(self, user_id: int) -> dict:
        ...
        try:
            # 서브쿼리들 정의
            followers_subq = select(func.count(UserFollowModel.follower_id)).where(
                UserFollowModel.following_id == user_id
            ).scalar_subquery()
            
            following_subq = select(func.count(UserFollowModel.following_id)).where(
                UserFollowModel.follower_id == user_id
            ).scalar_subquery()
            
            following_persons_subq = select(func.count(PersonFollowModel.person_id)).where(
                PersonFollowModel.user_id == user_id
            ).scalar_subquery()
            
            comments_subq = select(func.count(CommentModel.comment_id)).where(
                CommentModel.user_id == user_id
            ).scalar_subquery()
            
            liked_movies_subq = select(func.count(MovieLikeModel.movie_id)).where(
                MovieLikeModel.user_id == user_id
            ).scalar_subquery()
            
            watchlist_subq = select(func.count(WatchlistModel.movie_id)).where(
    ...
    async def _get_followers_list(self, user_id: int, limit: int) -> list[UserFollower]:
        ...
        try:
            stmt = select(
                ...
                UserFollowModel.following_id == user_id
            ).order_by(UserFollowModel.created_at.desc()).limit(limit)
    ...
    async def _get_following_persons_list(self, user_id: int, limit: int) -> list[UserFollowingPerson]:
        """팔로우 중인 인물 목록"""
        try:
            stmt = select(
                ...
                PersonFollowModel.user_id == user_id
            ).order_by(PersonFollowModel.created_at.desc()).limit(limit)
    ...
    async def _get_recent_comments(self, user_id: int, limit: int) -> list[UserComment]:
        ...
        try:
            stmt = select(
                ...
                CommentModel.is_spoiler,
                CommentModel.created_at
            ).order_by(CommentModel.created_at.desc()).limit(limit)
    ...
    async def _get_liked_movies(self, user_id: int, limit: int) -> list[WatchlistMovie]:
        ...
        try:
            stmt = select(
                ...
                MovieLikeModel.user_id == user_id
            ).order_by(MovieLikeModel.created_at.desc()).limit(limit)
    ...
    async def _get_watchlist_movies(self, user_id: int, limit: int) -> list[WatchlistMovie]:
        ...
        try:
            stmt = select(
                ...
                WatchlistModel.user_id == user_id
            ).order_by(WatchlistModel.created_at.desc()).limit(limit)
