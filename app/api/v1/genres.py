...
async def get_all_genres(genre_service: GenreService = Depends(get_genre_service)):
    try:
...
async def get_genre_stats(genre_service: GenreService = Depends(get_genre_service)):
    try:
...
@router.get(
    "/tmdb/movies", summary="TMDB 장르 목록", description="TMDB에서 영화 장르 목록을 가져옵니다."
)
...
async def sync_genres_from_tmdb(
    ...
    try:
        ...
        for tmdb_genre in tmdb_genres.get("genres", []):
            genre = await genre_service.create_genre(tmdb_genre["id"], tmdb_genre["name"])
